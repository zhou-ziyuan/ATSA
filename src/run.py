...
def run_sequential(args, logger):
    ...
    scheme = {
        ...
        "perturbations": {"vshape": env_info["obs_shape"], "group": "agents"},  # Store adversarial perturbations when needed.
    }
    groups = {
    ...
    if args.Number_attack > 0:
        if args.attack_method == "fgsm" or args.attack_method == "pgd" or args.attack_method == "rand_noise" or args.attack_method == "gaussian":
            ...
            mac = mac_REGISTRY[args.mac](buffer.scheme, groups, args)
            runner.setup(scheme=scheme, groups=groups, preprocess=preprocess, mac=mac)
            learner = le_REGISTRY[args.learner](mac, buffer.scheme, logger, args)  # Victim multi-agent learner.
            args.obs_shape = env_info["obs_shape"]
            adv_learner = MAPPO(args)  # Learner used to generate perturbations.
        
        elif args.attack_method == "harl_paad":
            buffer = ReplayBuffer(scheme, groups, args.buffer_size, env_info["episode_limit"] + 1,
                                  preprocess=preprocess,
                                  device="cpu" if args.buffer_cpu_only else args.device)
    ...
    if args.use_cuda:
        learner.cuda()
        if args.Number_attack > 0 and (args.attack_method == "paad" or args.attack_method == "atsa"):
            adv_learner.cuda()
    ...
    if args.adv_checkpoint_path != "" and args.Number_attack > 0 and (args.attack_method == "paad" or args.attack_method == "atsa" or args.attack_method == "atla"):
    ...
    while runner.t_env <= args.t_max:
        if args.Number_attack > 0:
            if args.attack_method == "fgsm" or args.attack_method == "pgd" or args.attack_method == "rand_noise" or args.attack_method == "gaussian":
                ...
                # If MAPPO is used to generate perturbations, the runner must pass the perturbation model output into the victim policy.
