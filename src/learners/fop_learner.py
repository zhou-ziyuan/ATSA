...
class FOP_Learner:
    def __init__(self, mac, vic_mac, scheme, logger, args):
        ...
        # Optimizing log(alpha) is numerically more stable than optimizing alpha directly.
    ...
    def train_actor(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Keep the categorical policy well-formed after masking invalid actions.
        ...
        # Discrete SAC-style actor objective.
    ...
    def train_actor_loss(self, batch: EpisodeBatch, vic_batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            ...
            # print(target_logits, adv_actions[0])
        ...
        # Discrete SAC-style actor objective with an additional regularization term.
        pol_target = (pi * (alpha * log_pi - q_vals[:,:-1].reshape(-1, self.n_actions))).sum(dim=-1)
